...
function al_dump() {
	...
	$path = is_dir( '/scr' ) ? '/scr/mp-p1-alarm.txt' : '/tmp/mp-p1-alarm.txt';
...
$logger->log_exception( new RuntimeException( 'test 1.19 — poczta dziala' ), new MP_Context( array( 'lead_id' => null ) ), 7 );
...
$logger->log_exception( new RuntimeException( 'test 1.19 — poczta odrzuca' ), new MP_Context( array( 'lead_id' => null ) ), 7 );
...
$wynik = MP_Result::fail( 'test 1.19', array(), 'test_code' );

delete_transient( 'mp_notify_' . $dzial->get_key() );
$przed = al_ile_porazek();

add_filter( 'pre_wp_mail', '__return_false', 99 );
$logger->log_failure( $dzial, $wynik, new MP_Context( array( 'lead_id' => null ) ) );
remove_filter( 'pre_wp_mail', '__return_false', 99 );
